...
### Purpose
Aggregates all sales activity per product and enriches it with profitability metrics, performance classification, and engagement indicators.
...
- Joins `fact_sales` with `dim_products` using a `LEFT JOIN` to retain all sales records regardless of whether product data is complete.
- Brings in product attributes (`category`, `subcategory`, `cost`) alongside transactional fields for downstream aggregation.
- Filters out rows where `order_date` is `NULL` to work only with valid transactions.
...
|---|---|
| `total_orders` | Number of distinct orders containing this product |
| `last_order_date` | Most recent date the product was purchased |
| `lifespan` | Months between first and last sale — measures how long the product has been active |
| `total_cost` | Sum of unit costs across all sales — used to estimate total cost of goods sold |
| `total_sales` | Total revenue generated by the product |
| `total_quantity` | Total units sold |
| `total_customers` | Number of distinct customers who purchased this product |
| `avg_selling_price` | Average revenue per unit sold. `CAST` to `FLOAT` ensures decimal precision. `NULLIF(quantity, 0)` prevents division by zero. Result is rounded to 2 decimal places. |
...
**`product_performance`**  
Classifies each product into a performance tier based on total revenue generated:
- **Low Performance** — Total sales below $100,000
- **Mid Range** — Total sales between $100,000 and $500,000
- **High Performance** — Total sales above $500,000

**`recency`**  
Months elapsed since the product's last sale. Helps identify products that are no longer being purchased — useful for inventory and catalog decisions.

**`avg_order_revenue`**  
Average revenue per order for this product. Division-by-zero is handled with a `CASE` guard.

**`avg_monthly_revenue`**  
Average revenue generated per active month. For products where `lifespan = 0` (first and last sale in the same month), total sales is returned as-is to avoid division by zero.
